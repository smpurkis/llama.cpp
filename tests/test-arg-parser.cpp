...
static void test(void) {
    ...
    assert(params.use_mlock == true);

    argv = {"binary_name", "--mmap"};
    assert(true == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, LLAMA_EXAMPLE_COMMON));
    ...
    argv = {"binary_name", "--direct-io"};
    assert(true == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, LLAMA_EXAMPLE_COMMON));
    assert(params.use_direct_io == true);
...
// skip this part on windows, because setenv is not supported
    ...
    setenv("LLAMA_ARG_MMAP", "1", true);
    argv = {"binary_name"};
    assert(true == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, LLAMA_EXAMPLE_COMMON));
    assert(params.use_mmap == true);

    setenv("LLAMA_ARG_MLOCK", "1", true);
    argv = {"binary_name"};
    assert(true == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, LLAMA_EXAMPLE_COMMON));
    assert(params.use_mlock == true);
    ...
    setenv("LLAMA_ARG_MMAP", "", true); // invalid value -> should fail
    argv = {"binary_name"};
    assert(false == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, LLAMA_EXAMPLE_COMMON));
